...
const MusicSectionContent2: React.FC<MusicSectionContentProps> =({thumb,artist_name, name,audio,id,index})=>{
  ...
  useEffect(() => {
    if (currentSongIdForPagination === id) {
      setIsActive(true);
      if (isPlaying) {
        play(); // Only play if the state is set to playing
      }
    } else if (currentSongIdForPagination !== id) {
      setIsActive(false);
      stop();
      setIsPlaying(false); // Ensure audio is stopped
    }
  }, [currentSongIdForPagination, isPlaying]); // Depend on isPlaying to manage play state
  ...
  return (
    <>
      <div key={`${index} + ${id}`} className="flex justify-between">
        <div className={`w-[255px] h-[365px] bg-black bg-contain bg-no-repeat rounded-xl mr-5 relative`}>
          <div className="p-3">
            <img src={thumb} />
          </div>
              ...
              {!isPlaying ? (
                <PlayIcon
                  ...
                  onClick={() => {
                    ...
                    setIsActive(true);
                    setIsPlaying(true); // Set state to playing when play is clicked
                    play();
                  }}
                />
              ) : (
                <PauseIcon
                  ...
                  onClick={() => {
                    stop();
                    setIsPlaying(false); // Set state to paused when pause is clicked
                    setIsActive(false);
                  }}
                />
              )}
          ...
          <div className="w-full px-5 mt-6">
            <p className="text-[#FB8A2E] text-[10px]">New Track for you</p>
            <p className="font-bold text-[20px]">
              {artist_name?.replace(/<a[^>]*>([^<]*)<\/a>/gi, '$1')}
            </p>
            <p className="text-[12px] text-[#9898A6] font-semibold">{name}</p>
          </div>
